...
class Game {
    ...
    runGame(){
        ...
        startBtn.addEventListener('click', () => {
            this.player.timer();
            this.submarine.positionSubmarineInterval();
        });
        this.player.submarineAttack();
...
class Submarine {
    ...
    positionSubmarine(){
        ...
        for(let i = 0; i < submarineCard.length; i++){
            submarineCard[i].style.position = 'absolute';
            const newPosition = Math.random() * 500;
            submarineCard[i].style.left = `${newPosition}px`;
            submarineCard[i].style.top = `${newPosition}px`;
        }  
    }

    positionSubmarineInterval(){
        setInterval(this.positionSubmarine, 500)
    }
...
class Player {
    ...
    constructor(){
        ...
        this.submarineCardAttack = document.querySelectorAll('.team-card');
        this.submarineCardAttackBcg = document.querySelectorAll('.card-bcg');
        this.submarine = new Submarine();
    }
    ...
    submarineAttack(){
        this.counter = 0;
        for(let i = 0; i < this.submarineCardAttack.length; i++){
            this.submarineCardAttack[i].addEventListener('click', () => {
                this.submarineCardAttack[i].style.backgroundImage = 'url(../img/popeyecard.png)';
                this.submarineCardAttackBcg[i].style.display = 'none';
                this.submarine.gotHit(); 
                setInterval(() =>{
                    if(this.counter % 2 === 0){
                        this.submarineCardAttack[i].classList.add('none');
                        this.counter++;
                    } else {
                        this.submarineCardAttack[i].classList.remove('none');
                        this.counter++;
                    }
                }, 1000)                 
            });
    ...
    //     setInterval(() => {
    //         if(this.counter % 2 === 0){
    //             this.submarineCardAttack[1].style.backgroundImage = 'url(../img/teamphoto/agnieszka.png)';
    //             this.submarineCardAttack[2].style.backgroundImage = 'url(../img/teamphoto/kasia.png)';
    //             this.submarineCardAttack[3].style.backgroundImage = 'url(../img/teamphoto/mateusz.png)';
    //             this.submarineCardAttack[4].style.backgroundImage = 'url(../img/teamphoto/staszek.png)';
    //             this.submarineCardAttack[5].style.backgroundImage = 'url(../img/teamphoto/lukasz.png)';
    //             this.counter++;
